...
export const WHATSAPP_MESSAGE = (model: string, color: string, storage: string, ram: string, brand: string) => {
  const parts = [`${color}, ${formatStorage(storage)}`]
  if (!brand.toLowerCase().includes('apple')) parts.push(formatRam(ram))
